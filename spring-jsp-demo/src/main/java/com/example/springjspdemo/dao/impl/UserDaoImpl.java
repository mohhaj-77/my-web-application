...
public class UserDaoImpl extends JdbcDaoSupport implements UserDao {
	...
	public User getUserById(String userId) {
     String sql="select cast(aes_encrypt(unhex(`user_password`), 'secret') as char(50)) from user where `user_id`=?";
...
/*@Override
  public int createNewUser(User user) {
	String sql="insert into user(user_id, user_password) values(?, hex(aes_encrypt(?,'secret')))";
	 return getJdbcTemplate().update(new preparedStatmentCreator()){
		 
		 @Override
		 public preparedStatment CreatrepreparedStatment(Connection con throw SQLException{
			 return null;
		 }
	 });
  }
